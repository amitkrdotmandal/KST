...
No_of_last_months_to_analyze = int(input("Enter the number of last months to analyze to get avg. consumption per month: "))
...
df_month_combined_Odr['Total_order_of_all_months'] = 0
for i in range(1, num_of_columns_in_df_month_combined_Odr, 2):
    print(i)
    # print(df_month_combined_Odr.iloc[:, i])
    df_month_combined_Odr['Total_order_of_all_months'] = (
            df_month_combined_Odr['Total_order_of_all_months'].fillna(0)
            + df_month_combined_Odr.iloc[:, i].fillna(0)
    )


df_month_combined_Odr['Avg_order_for_item_per_month_in_kg'] =df_month_combined_Odr['Total_order_of_all_months']/((num_of_columns_in_df_month_combined_Odr-1)/2)
df_month_combined_Odr.rename(columns={'Item Mapped': 'item mapped'}, inplace=True)

df_month_combined_Odr.to_excel('Output_With_ply_Item_vs_Odr_in_Kg_Monthly.xlsx', index=False)
