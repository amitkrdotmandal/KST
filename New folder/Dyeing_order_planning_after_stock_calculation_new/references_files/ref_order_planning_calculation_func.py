...
def order_planning_calculation_func(Article_no_to_plan,Shade_to_plan,Qty_to_plan_in_no_for_cones_or_box,Order_already_punched_in_PDMS):
    # Article_no_to_plan = 3302
    # Shade_to_plan = 'N-BLACK'
    # Qty_to_plan_in_no_for_cones_or_box = 160
    # Order_already_punched_in_PDMS = 1  ### If already punched '1', if not already punched '0'
    ...
    if not match.empty:
        ...
        total_weight_for_planned_odr_in_kg = Qty_to_plan_in_no_for_cones_or_box * (
        match.iloc[0]['Dry weight(gram)']) / 1000
    ...
    finish_stock_weight = 0
    match = df_ProductionPlanning_aggregated[
        (df_ProductionPlanning_aggregated['Item Name according to me'].astype(str) == str(
            Item_Name_according_to_me_for_order_plan)) & (
    ...
    if not match.empty:
        total_prod_req_in_KG_IN_existing_production_plan = total_prod_req_in_KG_IN_existing_production_plan + \
                                                           match.iloc[0]['total_prod_req_in_KG']
        finish_stock_weight = match.iloc[0]['total_prod_req_in_KG']
    ...
    match = df_jobcard_pkg_streaching[
        (df_jobcard_pkg_streaching['Item Name according to me'].astype(str) == str(
            Item_Name_according_to_me_for_order_plan)) & (
    ...
    match = df_jobcard_pkg_winding[
        (df_jobcard_pkg_winding['Item Name according to me'].astype(str) == str(
            Item_Name_according_to_me_for_order_plan)) & (
    ...
    match = df_rptstapprovalpending[
        (df_rptstapprovalpending['Item Name according to me'].astype(str) == str(
            Item_Name_according_to_me_for_order_plan)) & (
    ...
    match = df_daily_dyeing_order_to_despatch_report[
        (df_daily_dyeing_order_to_despatch_report['Item Name according to me'].astype(str) == str(
            Item_Name_according_to_me_for_order_plan)) & (
    ...
    print('Finish_stock in Kg before adjustment:', finish_stock_weight)
    print('Finish_stock Qty in cone or boxes:', int(finish_stock_weight / Dry_weight_of_cone_or_box * 1000))
    print('prod_plan in Kg AFTER adjustment:', total_prod_req_in_KG_IN_existing_production_plan)
    ...
    print('For Article ', Article_no_to_plan, ' & shade ', Shade_to_plan, ' with order qTY of ',
          Qty_to_plan_in_no_for_cones_or_box, ', the Qty to plan for dyeing in KG is :',
          Order_need_to_plan_for_dyeing_KG)
    return Order_need_to_plan_for_dyeing_KG
...
# Qty_to_plan_in_no_for_cones_or_box = 160
# Order_already_punched_in_PDMS = 1  ### If already punched '1', if not already punched '0'
# order_planning_calculation_func(Article_no_to_plan,Shade_to_plan,Qty_to_plan_in_no_for_cones_or_box,Order_already_punched_in_PDMS)